...
document.getElementById("btn4").addEventListener("click", function () {
  ...
  document.getElementById("btn2").style.backgroundColor = "white";
  document.getElementById("btn3").style.backgroundColor = "white";
  document.getElementById("btn4").style.backgroundColor = "#f6f6f6";
  document.getElementById("btn5").style.backgroundColor = "white";
});
document.getElementById("btn5").addEventListener("click", function () {
  ...
  document.getElementById("btn3").style.backgroundColor = "white";
  document.getElementById("btn4").style.backgroundColor = "white";
  document.getElementById("btn5").style.backgroundColor = "#f6f6f6";
});
...
document
  ...
  .addEventListener("click", async function (event) {
    ...
    // Create a FormData object to hold the album name and artist name
    const formData = new FormData();
    formData.append(
      "AlbumName",
      document.getElementById("RemovealbumName").value
    );
    formData.append("ArtistName", artistname); // Replace with the actual artist name

    try {
      // Send a DELETE request to the API to delete the album
      const deleteAlbumResponse = await fetch(
        "https://localhost:7259/api/Artist/DeleteAlbum",
        {
          method: "DELETE",
          body: formData,
        }
      ...
      if (!deleteAlbumResponse.ok) {
        const errorText = await deleteAlbumResponse.text();
        console.error(errorText);
        alert(`Error: ${errorText}`);
        return;
      }

      alert(await deleteAlbumResponse.text());

      // Fetch all albums for the artist again to refresh the list
      const getAlbumsResponse = await fetch(
        `https://localhost:7259/api/Artist/ArtistAlbums?artistName=${artistname}`
      );
      ...
      const albums = await getAlbumsResponse.json();
      populateAlbumsTable2(albums);
    } catch (error) {
      console.error("Error:", error);
      alert("An error occurred while processing your request.");
    }
  });
function populateAlbumsTable2(albums) {
  const tab = document.getElementById("albumsTable2");
  tab.style.opacity = "1";
  const tbody = document.getElementById("albumsTable2").querySelector("tbody");
  tbody.innerHTML = ""; // Clear existing rows
...
document
  ...
  .addEventListener("click", async function (event) {
    ...
    try {
      // delete the song
      const deleteSongResponse = await fetch(
        "https://localhost:7259/api/Artist/DeleteSong",
      ...
      if (!deleteSongResponse.ok) {
        const errorText = await deleteSongResponse.text();
        alert(`Error: ${errorText}`);
        return;
      }

      alert(await deleteSongResponse.text());
...
function populateSongsTable2(songs) {
  const tab2 = document.getElementById("songsTable2");
  tab2.style.opacity = "1";
  const tbody = document.getElementById("songsTable2").querySelector("tbody");
  tbody.innerHTML = ""; // Clear existing rows
